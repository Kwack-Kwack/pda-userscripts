...
(async () => {
    window.addEventListener("hashchange", (e) => {
        if (getHashParams(new URL(e.newURL).hash).get("option") === "stock")
            start();
    });
    if (getHashParams(document.location.hash).get("option") === "stock")
        start();
    ...
    function callback(form) {
        const storageCap = Array.from(form.querySelectorAll(".storage-capacity > *")).map(el => getNumber(el.innerText));
    ...
    function addButton(form) {
        if ($("#kw-auto-fill").length > 0)
            return; // Do not inject button twice
        $("<span/>", { class: "btn-wrap silver" })
            .append($("<span/>", { class: "btn" }).append($("<button/>", { class: "torn-btn", id: "kw-auto-fill" })
